...
const submitHandler = async () => {
  ...
  if (valid) {
    ...
    if (route.params.id) formData.append("id", formDataLocal.value.id);
    ...
    if (formDataLocal.value.role_id) {
...
onMounted(() => {
  callOrgRole();
  if (route.params.id) {
    api.post(`users-show`, { id: route.params.id }).then((res) => {
      formDataLocal.value.id = res.data.data.id;
      formDataLocal.value.username = res.data.data.username;
      formDataLocal.value.email = res.data.data.email;
      formDataLocal.value.school_id = res.data.data.school?.id;
      formDataLocal.value.role_id = res.data.data.roles[0].id;
      localImage.value = res.data.data.photo_path;
    });
  }
});
</script>
