...
<template>
  <VContainer id="printableReport">
    ...
    >
      <template #item.total_rentals="{ item }">
        {{ formatCurrency(item.total_rentals) }}
      </template>
    ...
    >
      <VCardTitle>{{ $t('Rental Chart') }}</VCardTitle>
      <VCardText>
...
const headers = [
  ...
  { title: t('Rental Price'), key: 'total_rentals', align: 'start' },
  { title: t('Grand Total'), key: 'total_rentals', align: 'start' },
]
...
const grandSubtotal = computed(() => reportData.value.reduce((sum, item) => sum + (item.total_rentals || 0), 0))

async function fetchReport() {
  loading.value = true
  try {
    const res = await api.get('/rental-report', { params: filters.value })
...
function exportExcel() {
  ...
  const wsData = [
    ...
    ...reportData.value.map(row => [row.period, row.total_rentals, row.grand_total]),
  ]
